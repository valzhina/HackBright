...
def merge_lists(list1, list2):
    ...
    # while there's still data in both lists, compare first elements and add
    # the smaller one to our output list
    ...
    # now that one of the lists is empty, add the rest of the other list to our
    # output list
    if len(list1) > 0:
        output.extend(list1)
    else:
...
def merge_sort(lst):
    ...
    if len(lst) < 2:
        ...
        list1 = lst[:int(len(lst) / 2)]
        list2 = lst[int(len(lst) / 2):]
